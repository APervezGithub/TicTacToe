...
class BoardSquare
{
  ...
  /** Display square */
  void Display(int Player, boolean ShouldHighlight)
  {
    switch(Player)
    {
      case -1:
...
class Board
{
  ...
  /** Set default values */
  void Initialize(int x_, int y_, int BoardNum_, int SquareSize_)
  {
    x = x_;
    y = y_;
  ...
  void IncrementTotals(int Player, int Index)
  {
  ...
  void CheckEnd(int Player, int xIndex, int yIndex)
  {
